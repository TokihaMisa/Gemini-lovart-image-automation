...
- `excel.columns`：商品字段列，包括商品 ID、中文名、语言、卖点，以及“参考图是否是产品图”的是/否标记。
...
| 目标语言 | C | 详情页输出语言 |
| 商品卖点 | D | 商品信息和卖点描述 |
| 参考图是否是产品图 | H | 填 `是` 表示后面的参考图是同一产品/规格/颜色/款式；填 `否` 表示只参考风格 |
...
- `output/<商品ID>/image_*.jpeg`：从 Excel 提取出的商品图片。第 1 张是产品图，第 2 张是配件图，第 3 张是尺寸图，第 4 张起是参考图；空位表示没有对应图片。若参考图前插入了“参考图是否是产品图”文字列，图片扫描会保留空位并继续读取后面的参考图。
